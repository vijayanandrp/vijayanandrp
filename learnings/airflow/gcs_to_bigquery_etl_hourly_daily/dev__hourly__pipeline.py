...
delete_partition_from_table_query = """DELETE `{project_id}.{dataset_id}.{raw_table_id}` WHERE partition_key = TIMESTAMP_TRUNC("{partition_key}", HOUR);"""
...
SELECT
    ...
    file_name,
    dt,
    TIMESTAMP_TRUNC("{partition_key}", HOUR) as partition_key
FROM
    `{temp_table_uri}`;
"""
